// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[cfg_attr(
    all(feature = "serde-deserialize"),
    derive(serde::Deserialize),
    serde(rename_all = "camelCase")
)]
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct GetVectorBucketPolicyInput {
    /// <p>The name of the vector bucket.</p>
    pub vector_bucket_name: ::std::option::Option<::std::string::String>,
    /// <p>The ARN of the vector bucket.</p>
    pub vector_bucket_arn: ::std::option::Option<::std::string::String>,
}
impl GetVectorBucketPolicyInput {
    /// <p>The name of the vector bucket.</p>
    pub fn vector_bucket_name(&self) -> ::std::option::Option<&str> {
        self.vector_bucket_name.as_deref()
    }
    /// <p>The ARN of the vector bucket.</p>
    pub fn vector_bucket_arn(&self) -> ::std::option::Option<&str> {
        self.vector_bucket_arn.as_deref()
    }
}
impl GetVectorBucketPolicyInput {
    /// Creates a new builder-style object to manufacture [`GetVectorBucketPolicyInput`](crate::operation::get_vector_bucket_policy::GetVectorBucketPolicyInput).
    pub fn builder(
    ) -> crate::operation::get_vector_bucket_policy::builders::GetVectorBucketPolicyInputBuilder
    {
        crate::operation::get_vector_bucket_policy::builders::GetVectorBucketPolicyInputBuilder::default()
    }
}

/// A builder for [`GetVectorBucketPolicyInput`](crate::operation::get_vector_bucket_policy::GetVectorBucketPolicyInput).
#[derive(
    ::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug,
)]
#[non_exhaustive]
pub struct GetVectorBucketPolicyInputBuilder {
    pub(crate) vector_bucket_name: ::std::option::Option<::std::string::String>,
    pub(crate) vector_bucket_arn: ::std::option::Option<::std::string::String>,
}
impl GetVectorBucketPolicyInputBuilder {
    /// <p>The name of the vector bucket.</p>
    pub fn vector_bucket_name(
        mut self,
        input: impl ::std::convert::Into<::std::string::String>,
    ) -> Self {
        self.vector_bucket_name = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The name of the vector bucket.</p>
    pub fn set_vector_bucket_name(
        mut self,
        input: ::std::option::Option<::std::string::String>,
    ) -> Self {
        self.vector_bucket_name = input;
        self
    }
    /// <p>The name of the vector bucket.</p>
    pub fn get_vector_bucket_name(&self) -> &::std::option::Option<::std::string::String> {
        &self.vector_bucket_name
    }
    /// <p>The ARN of the vector bucket.</p>
    pub fn vector_bucket_arn(
        mut self,
        input: impl ::std::convert::Into<::std::string::String>,
    ) -> Self {
        self.vector_bucket_arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The ARN of the vector bucket.</p>
    pub fn set_vector_bucket_arn(
        mut self,
        input: ::std::option::Option<::std::string::String>,
    ) -> Self {
        self.vector_bucket_arn = input;
        self
    }
    /// <p>The ARN of the vector bucket.</p>
    pub fn get_vector_bucket_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.vector_bucket_arn
    }
    /// Consumes the builder and constructs a [`GetVectorBucketPolicyInput`](crate::operation::get_vector_bucket_policy::GetVectorBucketPolicyInput).
    pub fn build(
        self,
    ) -> ::std::result::Result<
        crate::operation::get_vector_bucket_policy::GetVectorBucketPolicyInput,
        ::aws_smithy_types::error::operation::BuildError,
    > {
        ::std::result::Result::Ok(
            crate::operation::get_vector_bucket_policy::GetVectorBucketPolicyInput {
                vector_bucket_name: self.vector_bucket_name,
                vector_bucket_arn: self.vector_bucket_arn,
            },
        )
    }
}
